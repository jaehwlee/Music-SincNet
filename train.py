...
parser.add_argument('--batch-size', type=int, default=14, metavar='N', help='Mini-batch size.')
parser.add_argument('--momentum', type=float, default=0.9, metavar='M', help='Momentum for SGD.')
parser.add_argument('--lr', type=float, default=0.0000032, metavar='LR', help='Learning rate.')
...
parser.add_argument('--sr', type=int, default=22050, metavar='N', help='Sampling rate to set maximum frequency range of music-sinc filterbank')
parser.add_argument('--filter_size',type=int, default=2501, metavar='N', help='Filter size of the MusicSincLayer') 
...
def train(initial_lr, stage_train_dir, ckpt_path=None, initial_epoch=0):
    ...
    out = music_sincnet(x_train,filter_size=args.filter_size, filter_num=256, sr=args.sr, amplifying_ratio=args.alpha, drop_rate=args.dropout, weight_decay=args.weight_decay)
    ...
    model.compile_tfrecord(optimizer=optimizer, loss='binary_crossentropy', y=y_train, y_val=y_val)
    

    tensor_board = TensorBoard(log_dir=stage_train_dir)
    early_stopping = EarlyStopping(monitor='val_loss', patience=args.patience)
    checkpointer = ModelCheckpoint(make_path(stage_train_dir, 'ckpt-e{epoch:03d}-l{val_loss:.4f}.hdf5'),monitor='val_loss', save_best_only=True)
    ...
    model.fit_tfrecord(epochs=100, initial_epoch=initial_epoch, steps_per_epoch=train_steps, validation_steps=val_steps,callbacks=[tensor_board, early_stopping, checkpointer, csv_logger])
