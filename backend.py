...
def learning_phase_scope(value):
  ...
  """
    ...
    with ops.init_scope():
        if context.executing_eagerly():
            previous_eager_value = _GRAPH_LEARNING_PHASES.get(_DUMMY_EAGER_GRAPH, None)
...
def _scratch_graph(graph=None):
  ...
  """
    ...
    if (
        _CURRENT_SCRATCH_GRAPH is not None
        and graph is not None
        and _CURRENT_SCRATCH_GRAPH is not graph
    ):
    ...
    with get_graph().as_default():
        if sparse:
            ...
            x = ragged_factory_ops.placeholder(
                dtype=dtype, ragged_rank=ragged_rank, value_shape=value_shape, name=name
            )
        else:
...
def freezable_variable(value, shape=None, name=None):
  ...
  """
    graph = get_graph()
    with graph.as_default():
        x = array_ops.placeholder_with_default(value, shape=shape, name=name)
...
def batch_dot(x, y, axes=None):
  ...
  """
    ...
    if x_ndim > y_ndim:
        diff = x_ndim - y_ndim
        y = array_ops.reshape(
            y, array_ops.concat([array_ops.shape(y), [1] * (diff)], axis=0)
        )
    elif y_ndim > x_ndim:
        diff = y_ndim - x_ndim
        x = array_ops.reshape(
            x, array_ops.concat([array_ops.shape(x), [1] * (diff)], axis=0)
        )
    else:
        diff = 0
  ...
  """
    ...
    def __init__(self, inputs, outputs, updates=None, name=None):
        ...
        if updates and not outputs:
            ...
            raise ValueError(
                "Cannot create a Keras backend function with updates"
        ...
        if len(graphs) > 1:
            raise ValueError(
                "Cannot create an execution function which is comprised "
        ...
        with _scratch_graph() as exec_graph:
            ...
            if source_graph is global_graph and exec_graph is not global_graph:
                init_tensors = (
                    outputs
                    + updates_ops
                    + [p for [p, _] in legacy_update_ops]
                    + [
                        p_new
                        for [_, p_new] in legacy_update_ops
                ...
                legacy_update_ops = [
                    (lifted_map[p], lifted_map.get(p_new, p_new))
                ...
                for old_op, new_op in lifted_map.items():
                    if old_op in freezable_vars_from_keras_graph:
                        ...
                        if frozen_var._initial_value != frozen_var._current_value:
                            ...
                            self._freezable_vars_values.append(
                                frozen_var._current_value
                            )
        ...
        with exec_graph.as_default():
            for x in self.inputs:
                if x.op.type == "PlaceholderWithDefault":
                    self._placeholder_default_values[
                        ops.tensor_id(x)
                    ] = tensor_util.constant_value(x.op.inputs[0])
            ...
            for i in range(time_steps):
                ...
                for state, new_state in zip(states, new_states):
                    ...
                    return_states.append(
                        array_ops.where(tiled_mask_t, new_state, state)
                    )
...
def categorical_crossentropy(target, output, from_logits=False, axis=-1):
  ...
  """
    if not from_logits:
        if (
            isinstance(output, (ops.EagerTensor, variables_module.Variable))
            or output.op.type != "Softmax"
        ):
  ...
  """
    ...
    for position in itertools.product(*output_axes_ticks):
        ...
        slices.extend(
            [
                slice(
                    position[d] * strides[d], position[d] * strides[d] + kernel_size[d]
                )
