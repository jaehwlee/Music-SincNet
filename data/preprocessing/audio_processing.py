...
def _segments_to_sequence_example(segments, labels):
  ...
  """
    ...
    context = tf.train.Features(
        feature={"raw_labels": _bytes_feature(raw_labels)}  # uint8 Tensor (50,)
    )

    feature_lists = tf.train.FeatureLists(
        feature_list={
            "raw_segments": _bytes_feature_list(
                raw_segments
...
def _audio_to_segments(filename, sample_rate, num_samples):
  ...
  """
    ...
    sig = Signal(
        filename, sample_rate=sample_rate, dtype=np.float32, stop=29, num_channels=1
    )
...
def audio_to_sequence_example(filename, labels, sample_rate, num_samples):
  ...
  Returns:
    A SequenceExample proto.
  """
    segments = _audio_to_segments(
        filename, sample_rate=sample_rate, num_samples=num_samples
    )
