...
def batch_inputs(
    ...
    tf.logging.info(
        "Prefetching values from %d files matching %s", len(data_files), file_pattern
    )
    ...
    if is_training:
        ...
        example_list = [
            read_example_fn(filename_queue) for _ in range(num_read_threads)
        ]
        ...
        segment_batch, label_batch = tf.train.batch(
            [segment, label], batch_size, 1, capacity, name="fifo_" + example_queue_name
        )
...
def _read_sequence_example(filename_queue, n_labels=50, n_samples=59049, n_segments=10):
    ...
    context, sequence = tf.parse_single_sequence_example(
        serialized_example,
        context_features={"raw_labels": tf.FixedLenFeature([], dtype=tf.string)},
        sequence_features={
