...
def calculate_steps(n_examples, n_segments, batch_size):
    steps = 1.0 * n_examples * n_segments
...
class TFRecordModel(Model):
    ...
    """
        ...
        if do_validation and y_val is None:
            raise ValueError(
                "When you use validation inputs, " "you should provide y_val."
            )
        ...
        for i in range(len(self.outputs)):
            ...
            for metric in output_metrics:
                if metric == "accuracy" or metric == "acc":
                    ...
                    if (
                        output_shape[-1] == 1
                        or self.loss_functions[i] == losses.binary_crossentropy
                    ):
                        # case: binary accuracy
                        acc_fn = metrics_module.binary_accuracy
                    elif (
                        self.loss_functions[i] == losses.sparse_categorical_crossentropy
                    ):
